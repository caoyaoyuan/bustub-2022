...
auto B_PLUS_TREE_INTERNAL_PAGE_TYPE::Lookup(const KeyType &key, const KeyComparator &comparator) const -> ValueType {
  auto res = std::lower_bound(array_ + 1, array_ + GetSize(), key,
                              [&comparator](const auto &p, auto k) { return comparator(p.first, k) < 0; });

  if (res == array_ + GetSize()) {
    return ValueAt(GetSize() - 1);
  }

  if (comparator(res->first, key) == 0) {
    return res->second;
  }
...
auto B_PLUS_TREE_INTERNAL_PAGE_TYPE::InsertNodeAfter(const ValueType &old_value, const KeyType &new_key,
  ...
  array_[new_index] = {new_key, new_value};
  IncreaseSize(1);
  return GetSize();
}
...
void B_PLUS_TREE_INTERNAL_PAGE_TYPE::MoveHalfTo(BPlusTreeInternalPage *recipient,
  ...
  SetSize(start_idx);
  recipient->CopyNFrom(array_ + start_idx, old_size - start_idx, buffer_pool_manager);
}
